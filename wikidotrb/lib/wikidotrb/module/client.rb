...
module Wikidotrb
  ...
  class ClientPrivateMessageMethods
    ...
    def send(recipient, subject, body)
      Wikidotrb::Module::PrivateMessage.send(@client, recipient, subject, body)
    end

    # 受信箱を取得する
    # @return [PrivateMessageInbox] 受信箱
    def get_inbox
      Wikidotrb::Module::PrivateMessageInbox.acquire(@client)
    end

    # 送信箱を取得する
    # @return [PrivateMessageSentBox] 送信箱
    def get_sentbox
      Wikidotrb::Module::PrivateMessageSentBox.acquire(@client)
    end
    ...
    def get_messages(message_ids)
      Wikidotrb::Module::PrivateMessageCollection.from_ids(@client, message_ids)
    end
    ...
    def get_message(message_id)
      Wikidotrb::Module::PrivateMessage.from_id(@client, message_id)
    end
  end
  ...
  class ClientSiteMethods
    ...
    def get(unix_name)
      Wikidotrb::Module::Site.from_unix_name(@client, unix_name)
    end
  end
  ...
  class Client
    ...
    def initialize(username: nil, password: nil, amc_config: nil, logging_level: 'WARN')
      ...
      # AMCClientを初期化
      @amc_client = Wikidotrb::Connector::AjaxModuleConnectorClient.new(site_name: nil, config: amc_config)
    ...
    def login_check
      unless @is_logged_in
        raise Wikidotrb::Common::LoginRequiredException.new("Login is required to execute this function")
      end
      nil
    end
  end
