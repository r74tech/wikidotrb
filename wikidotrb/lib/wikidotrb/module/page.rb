...
module Wikidotrb
  module Module
    ...
    class SearchPagesQuery
      ...
      def to_h
        res = instance_variables.map { |var| [var.to_s.delete('@'), instance_variable_get(var)] }.to_h
    ...
    class Page
      ...
      def discuss
        if @_discuss.nil?
          PageCollection.new(site: @site, pages: [self]).get_page_discuss
        end
        @_discuss.update
        @_discuss
      end
      ...
      def id
        if @_id.nil?
          PageCollection.new(site: @site, pages: [self]).get_page_ids
        end
        @_id
      end
      ...
      def source
        if @_source.nil?
          PageCollection.new(site: @site, pages: [self]).get_page_sources
        end
        @_source
      end
      ...
      def revisions
        if @_revisions.nil?
          PageCollection.new(site: @site, pages: [self]).get_page_revisions
        end
      ...
      def votes
        if @_votes.nil?
          PageCollection.new(site: @site, pages: [self]).get_page_votes
        end
        @_votes
      end
