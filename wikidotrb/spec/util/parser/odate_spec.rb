...
RSpec.describe Wikidotrb::Util::Parser::ODateParser do
  describe ".parse" do
    context "正しいodate要素が渡された場合" do
      ...
      let(:expected_time) { Time.at(1_633_036_800) }
    ...
    context "unix timeのクラスが含まれているものの、Invalidなodate要素が渡された場合" do
      ...
      it "UnexpectedExceptionを発生させること" do
        expect do
          described_class.parse(elem)
        end.to raise_error(Wikidotrb::Common::Exceptions::UnexpectedException, "Invalid unix time")
      end
    end
    ...
    context "unix timeのクラスが含まれていないodate要素が渡された場合" do
      ...
      it "ArgumentErrorを発生させること" do
        expect do
          described_class.parse(elem)
        end.to raise_error(ArgumentError, "odate element does not contain a valid unix time")
      end
    end
