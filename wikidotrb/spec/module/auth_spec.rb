...
RSpec.describe Wikidotrb::Module::HTTPAuthentication do
  ...
  describe '.login' do
    let(:response) { instance_double('HTTPX::Response', status: 200, body: '', headers: { 'set-cookie' => 'WIKIDOT_SESSION_ID=123456' }) }
    ...
    context 'HTTPステータスコードが200以外の場合' do
      ...
      it 'SessionCreateExceptionが発生すること' do
        ...
        }.to raise_error(Wikidotrb::Common::Exceptions::SessionCreateException, /HTTP status code: 401/)
      end
    end
    ...
    context '無効なユーザー名またはパスワードが渡された場合' do
      ...
      it 'SessionCreateExceptionが発生すること' do
        expect {
          described_class.login(client, username, password)
        }.to raise_error(Wikidotrb::Common::Exceptions::SessionCreateException, /invalid username or password/)
      end
    end
    ...
    context 'クッキーが含まれない場合' do
      ...
      it 'SessionCreateExceptionが発生すること' do
        expect {
          described_class.login(client, username, password)
        }.to raise_error(Wikidotrb::Common::Exceptions::SessionCreateException, /invalid cookies/)
      end
    end
  end
  ...
  describe '.logout' do
    ...
    it 'ログアウトしてセッションが削除されること' do
      ...
      expect(amc_client).to have_received(:request).with([{ 'action' => 'Login2Action', 'event' => 'logout', 'moduleName' => 'Empty' }])
